...
import { deleteHardSupplierApi } from '@/services/supplier/supplier.api'
...
export function DeletePermanentSupplierDialog({ id, name }: { id: string; name: string }) {
  const { t } = useTranslation('supplier')
  const queryClient = useQueryClient()

  const mutation = useMutation({
    mutationFn: () => deleteHardSupplierApi(id),
    onSuccess: () => {
