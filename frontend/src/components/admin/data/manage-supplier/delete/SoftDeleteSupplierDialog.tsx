...
import { deleteSoftSupplierApi } from '@/services/supplier/supplier.api'
...
export function DeleteSupplierDialog({ open, onOpenChange, supplier }: DeleteSupplierDialogProps) {
  ...
  const mutation = useMutation({
    mutationFn: () => deleteSoftSupplierApi(supplier.id),
    onSuccess: () => {
    ...
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            {t('message.success.deleteConfirm', { name: supplier.name })}
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
